...
func TestStickyProgressSetCurrentSeedsPkgStart(t *testing.T) {
	sp := NewStickyProgress(5)
	assert.True(t, sp.pkgStart.IsZero())
	sp.SetCurrent("wget")
	assert.False(t, sp.pkgStart.IsZero())
}

func TestStickyProgressPkgElapsed(t *testing.T) {
	sp := NewStickyProgress(5)
	// No package set yet → empty string.
	assert.Equal(t, "", sp.pkgElapsed())

	// pkgStart in the past → non-empty.
	sp.pkgStart = time.Now().Add(-3 * time.Second)
