...
var DefaultCategories = []PrefCategory{
	...
	},
	{
		...
		Prefs: []Preference{
			...
			// Menu-bar dropdown mode for modules with a 3-way picker (Always Show /
			// Show When Active / Don't Show). macOS Sequoia stores this in the
			// ByHost scope; writing to the main domain is a silent no-op.
			// Values empirically determined on macOS Sequoia:
			//   2  = Show When Active (also the macOS default when key is absent)
			//   8  = Don't Show in Menu Bar
			//   18 = Always Show in Menu Bar
			// Only Sound is mapped here — Bluetooth/WiFi/Battery use a
			// different scheme (checkbox-only, no 3-way dropdown) that needs
			// its own investigation; tracked separately.
			{Domain: "com.apple.controlcenter", Key: "Sound", Type: "int", Value: "18", Desc: "Always show Sound in menu bar (not only when active)", Host: "currentHost"},
		},
	},
	{
		Name: "Security",
